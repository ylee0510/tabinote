...
カード上の投稿者名、およびコメント欄の投稿者名をタップ／クリックすると、その人の **公開旅行カード一覧** を表示する。

- **画面**: 新しい view（例 `view="userProfile"`）。対象ユーザーを `profileUserId` で保持
- **取得**: `feed` を `ownerId == profileUserId` かつ `publishedAt` 降順で取得（Firestore の複合インデックス `ownerId + publishedAt` が必要 → コンソール作業として明記）
- **ヘッダー**: アバター（`ownerPhoto` か頭文字）＋ニックネーム＋公開件数。フォロー等は持たない（v1）。表示するニックネームは最新の feed エントリの `ownerName` を採用
- **本文**: タイムラインと同じカードUIを再利用。カードのタップで従来どおり閲覧専用詳細へ
- **導線**: タイムラインのカード投稿者名、閲覧専用詳細のカード投稿者名、コメント欄の投稿者名 ── いずれもタップでこの画面へ。自分の名前をタップした場合は自分の公開カード一覧が出る
- **未ログインでも閲覧可**（feed は公開読み取りのため）
- **戻る**: タイムラインへ戻る導線を用意
...
- ユーザー公開プロフィール画面（`view="userProfile"` / `profileUserId`）と投稿者名タップ導線
...
- 複合インデックス `feed`（`ownerId` 昇順 + `publishedAt` 降順）追加（ユーザー公開プロフィール用）
